...
function renderResults(){
  ...
  if(results.length>1){const all=button('全選','quiet',selectAll);all.dataset.selectAll='';actions.append(all);if(query.kind==='bible')actions.append(button('複製整段','quiet',()=>copy(results,'passage')));actions.append(button('•••','icon',resultMenu,'結果選項'));}head.append(actions);$('#results').append(head);
...
function toggle(index,shift=false){const v=results[index];if(shift&&anchor!==null){const lo=Math.min(anchor,index),hi=Math.max(anchor,index);for(const entry of results.slice(lo,hi+1))selection.set(entry.id,entry);}else if(selection.has(v.id))selection.delete(v.id);else selection.set(v.id,v);anchor=index;track('verse_select',{search_id:searchId,book:v.book,chapter:v.chapter,verse_count:selection.size});syncSelection();}
function syncSelection(){document.querySelectorAll('.verse').forEach(row=>{const selected=selection.has(+row.dataset.id);row.classList.toggle('selected',selected);row.querySelector('.select-verse')?.setAttribute('aria-pressed',selected);});$('#selectionBar').hidden=!selection.size;$('#selectionCount').textContent=`已選 ${selection.size} 節`;document.querySelectorAll('[data-select-all]').forEach(b=>{const all=results.length>0&&selection.size===results.length;b.textContent=all?'全不選':'全選';b.setAttribute('aria-pressed',all);});}
function clear(){selection.clear();anchor=null;syncSelection();}
...
function resultMenu(){const c=panel('經文選項');const list=el('div','menu-list');list.append(button(selection.size===results.length?'全不選':'全選','',()=>{selectAll();$('#panel').close();}),button('複製全部經文','',()=>copy(results,'all')),button('複製此搜尋連結','',async()=>{try{await navigator.clipboard.writeText(location.href);toast('已複製連結');}catch{toast('請複製瀏覽器網址');}}),button('複製格式','',()=>openPanel('settings')));c.append(list);}
...
function openPanel(type){
  ...
  else if(type==='settings'){const c=panel('閱讀與複製');c.append(field('字體大小',[['small','較小'],['normal','標準'],['large','較大']],prefs.size,v=>{prefs.size=v;applyPrefs();save('ssb-prefs',prefs);}),field('外觀',[['system','跟隨系統'],['light','淺色'],['dark','深色']],prefs.theme,v=>{prefs.theme=v;applyPrefs();save('ssb-prefs',prefs);}),field('複製格式',[['paragraph','出處在前，經文分行'],['each','每節附出處'],['text','僅經文'],['legacy','連續經文合併出處（預設）']],prefs.format,v=>{prefs.format=v;save('ssb-prefs',prefs);preview.textContent=results.length?formatCopy(results.slice(0,2),prefs.format):'搜尋經文後可預覽複製格式。';}));const preview=el('pre','copy-preview',results.length?formatCopy(results.slice(0,2),prefs.format):'搜尋經文後可預覽複製格式。');c.append(preview);}
  else if(type==='help'){const c=panel('使用說明與隱私');const d=el('div','help');for(const t of ['直接輸入約3:16、太5:3-12、伯7.3-8.1、馬太福音5章，或任何關鍵字。','點左邊節號選取，再點一次取消。全選後可按「全不選」。電腦也可按住 Shift 點節號。連續經文會合併出處，每節之間空一行。','按 / 或 ⌘/Ctrl K 聚焦搜尋；選取後按 ⌘/Ctrl C 複製；Esc 取消選取。經文可用網址分享。','經文沿用原站和合本資料及標點。歷史、字體與複製設定只保存在這部裝置。','我們記錄匿名搜尋與複製彙總，改善查經體驗；不建立個人檔案，不在統計資料庫保存姓名、email 或 IP。短暫搜尋識別碼用於計算轉換並在兩天內清除。','GA4 用於整體流量、裝置及互動分析。搜尋內容不送入 GA4；網址中的搜尋詞也會從 GA4 頁面資料移除。'])d.append(el('p','',t));c.append(d,field('匿名使用統計',[['on','開啟'],['off','關閉']],(()=>{try{return localStorage.getItem('ssb-analytics')||'on';}catch{return 'on';}})(),v=>{try{localStorage.setItem('ssb-analytics',v);}catch{}location.reload();}));}
  else panel('複製經文');
}
